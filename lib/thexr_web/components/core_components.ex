defmodule ThexrWeb.CoreComponents do
  @moduledoc """
      ...
      >
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              id={"#{@id}-container"}
              class="bg-gray-900 shadow-zinc-700/10 ring-zinc-700/10 relative hidden rounded-2xl indigo-950 p-14 shadow-lg ring-1 transition"
            >
              <div id={"#{@id}-content"}>
  ...
  def simple_form(assigns) do
    ...
    <.form :let={f} for={@for} as={@as} {@rest}>
      <div class="mt-10 space-y-8 indigo-950">
    ...
    <div phx-feedback-for={@name}>
      <label class="flex items-center gap-4 text-sm leading-6 text-zinc-600">
        ...
        <input
          ...
          value="true"
          checked={@checked}
          class="rounded border-zinc-300 text-zinc-200 bg-black focus:ring-0"
          {@rest}
        />
        <%= @label %>
      </label>
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <select
        id={@id}
        name={@name}
        class="mt-1 block w-full rounded-md border border-gray-300 indigo-950 shadow-sm focus:border-zinc-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        name={@name}
        class={[
          "mt-2 block w-full rounded-lg text-zinc-200 focus:ring-0 sm:text-sm sm:leading-6",
    ...
    <div phx-feedback-for={@name}>
      ...
      <input
        ...
        class={[
          "mt-2 block w-full rounded-lg text-zinc-200 focus:ring-0 sm:text-sm sm:leading-6",
          "phx-no-feedback:border-zinc-300 phx-no-feedback:focus:border-zinc-400",
          "border-zinc-300 focus:border-zinc-400 bg-black",
          @errors != [] && "border-rose-400 focus:border-rose-400"
        ]}
        {@rest}
      />
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="w-[40rem] mt-11 sm:w-full">
        ...
        <tbody
          ...
          class="relative divide-y divide-zinc-100 border-t border-zinc-200 text-sm leading-6 text-zinc-200"
        >
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-purple-900">
            <td
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                ...
                <span
                  :for={action <- @action}
                  class="relative ml-4 font-semibold leading-6 text-zinc-200 hover:text-zinc-200"
                >
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-zinc-100">
        <div :for={item <- @item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
          <dt class="w-1/4 flex-none text-zinc-300"><%= item.title %></dt>
          <dd class="text-zinc-200"><%= render_slot(item) %></dd>
        </div>
      </dl>
    </div>
    """
  ...
  def back(assigns) do
    ...
    <div class="mt-16">
      <.link
        navigate={@navigate}
        class="text-sm font-semibold leading-6 text-zinc-200 hover:text-zinc-200"
      >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ThexrWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ThexrWeb.Gettext, "errors", msg, opts)
    end
  end
